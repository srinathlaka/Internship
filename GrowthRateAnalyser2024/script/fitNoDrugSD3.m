function [Data,parameters] = fitNoDrugSD3(Data,mu,x0,k,kdeath,tin,tdeath,tswitch,tfin,kswitch,parameters)
...
%initial guess
P0 = [parameters.mu,parameters.x0,parameters.k,parameters.kdeath,parameters.tdeath,parameters.tswitch,parameters.kswitch];
%eventual boundaries
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,Kmin,-inf,time(tdeath),time(tin),-inf];
ub = [inf,Xmax,Kmax,inf,time(tfin),time(tdeath),inf];

%minimize chi square
[fittedParameters,fval] = fminsearchbnd(@(P) minimizeChiSquareSD3(P,blackboxmodel,time,parameters,Data),P0,lb,ub,options);

%compute p value
p = 1 - chi2cdf(fval,size(fittedParameters,2));
fprintf('mu = %f, x0 = %f, k = %f, kdeath = %f, Tdeath = %f, Tswitch = %f, kswitch = %f, residual chi2 = %f, p = %f \n',fittedParameters(1),fittedParameters(2),fittedParameters(3),fittedParameters(4),fittedParameters(5),fittedParameters(6),fittedParameters(7),fval,p)
