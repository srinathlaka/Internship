function [Data,parameters] = fitLESES(Data,tin,tfin,parameters0,i,parameters)
...
if isnan(tswitch)
    ...
    param.tswitch = time(round(tin+( tfin-tin)/2));
else
    param.tswitch = time(tswitch);
end

%define the model
blackboxmodel = @ModelLESES;
...
%initial guess
P0 = [param.mu,param.x0,param.k,param.tswitch,param.kswitch,param.q0,param.k2];
%eventual boundaries
...
Kmin = min(X);
Kmax = max(X);
lb = [0,Xmin,Kmin,time(tin),-inf,0,Xmin]; %%%%%%%%%%%%%%%%
ub = [inf,Xmax,Kmax,time(tfin),inf,inf,Xmax];

%model chi square
modelChi = @(P) minimizeChiSquareLESES(P,blackboxmodel,time,param,Data);

%minimize chi square
%[fittedparameters,fval] = fminsearchbnd(@(P) minimizeChiSquareLSD3(P,blackboxmodel,time,param,Data),P0,lb,ub,options);
...
param.kswitch = fittedparameters(5);
param.q0 = fittedparameters(6);
param.k2 = fittedparameters(7);
...
parameters.s{i}(5) = fittedparametersStd(4);
parameters.s{i}(6) = fittedparametersStd(5);
parameters.s{i}(7) = NaN;
parameters.s{i}(8) = NaN;
parameters.s{i}(9) = fittedparametersStd(7);
