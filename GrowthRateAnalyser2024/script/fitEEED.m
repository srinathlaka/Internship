function [Data,parameters] = fitEEED(Data,tin,tfin,parameters0,i,parameters)
...
%[1 klag,2 tlag,3 q0,4 mu0,5 tswitch,6 kswitch,7 -,8 -,9 -,10 tdeath,11 kdeath,12 m, 13 x0,14 k]
...
%initial guess
P0 = [param.mu,param.x0,param.kdeath,param.tdeath,param.tswitch,param.kswitch,param.kswitch2,param.tswitch2,param.k,param.k2];
%eventual boundaries
xBoundaries = (X(X>0));%X(1:10);
Xmin = min(xBoundaries);
Xmax = max(xBoundaries);
lb = [-inf,Xmin,-inf,time(tswitch2),time(tin),-inf,-inf,time(tswitch),-inf,-inf];
ub = [inf,Xmax,inf,time(tfin),time(tswitch2),inf,inf,time(tdeath),inf,inf];

%model chi square
modelChi = @(P) minimizeChiSquareSD10(P,blackboxmodel,time,param,Data);

%minimize chi square
%[fittedparameters,fval] = fminsearchbnd(@(P) minimizeChiSquareSD10(P,blackboxmodel,time,param,Data),P0,lb,ub,options);
...
param.kswitch2 = fittedparameters(7);
param.tswitch2 = fittedparameters(8);
param.k = fittedparameters(9);
param.k2 = fittedparameters(10);
