function [Data,parameters] = fitLESE(Data,tin,tfin,parameters0,i,parameters)
...
%initial guess
P0 = [param.mu,param.x0,param.k,param.tswitch,param.kswitch,param.q0];
%eventual boundaries
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,Kmin,time(tin),-inf,0];
ub = [inf,Xmax,Kmax,time(tfin),inf,inf];

%model chi square
modelChi = @(P) minimizeChiSquareLESE(P,blackboxmodel,time,param,Data);
