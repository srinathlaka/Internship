function [Data,parameters] = fitLES(Data,tin,tfin,parameters0,i,parameters)
...
%initial guess
P0 = [param.mu,param.x0,param.k,param.q0];
%eventual boundaries
...
Kmax = max(X);
lb = [-inf,Xmin,Kmin,0];
ub = [inf,Xmax,Kmax,inf];

%model chi square
modelChi = @(P) minimizeChiSquareLSD3b(P,blackboxmodel,time,param,Data);

%minimize chi square
%[fittedparameters,fval] = fminsearchbnd(@(P) minimizeChiSquareLSD3b(P,blackboxmodel,time,param,Data),P0,lb,ub,options);
...
fprintf('mu = %f, x0 = %f, k = %f, q0 = %f, residual chi2/DOF = %f, p = %f \n',fittedparameters(1),fittedparameters(2),fittedparameters(3),fittedparameters(4),sigmaSq,p)
