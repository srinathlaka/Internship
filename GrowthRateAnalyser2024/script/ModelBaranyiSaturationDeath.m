function Xm = ModelBaranyiSaturationDeath(time,parameters)
...
Xm = ((x0.*((1 + q0.*exp(mu0.*time))./(1 + q0 - q0.*x0./k0 + (q0.*x0.*exp(mu0.*time))./k0)))).*(exp((-dm0.*(heaviside(time-tdeath))).*time));
%Xm = ((x0.*((1 + q0.*exp(mu0.*time))./(1 + q0 - q0.*x0./k0 + (q0.*x0.*exp(mu0.*time))./k0)))).*(exp((-dm0.*(sigmoid(time,tdeath(1)))).*time));
