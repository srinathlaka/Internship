function [Data,parameters] = fitLEES(Data,tin,tfin,parameters0,i,parameters)
...
%initial guess
P0 = [param.mu,param.x0,param.klag,param.kswitch,param.tdeath,param.tswitch,param.k];
%eventual boundaries
xBoundaries = (X(X>0));%X(1:10);
Xmin = min(xBoundaries);
Xmax = max(xBoundaries);
lb = [-inf,Xmin,-inf,-inf,time(tswitch),time(tin),-inf];
ub = [inf,Xmax,inf,inf,time(tfin),time(tdeath),inf];

%model chi square
modelChi = @(P) minimizeChiSquareLEES(P,blackboxmodel,time,param,Data);
