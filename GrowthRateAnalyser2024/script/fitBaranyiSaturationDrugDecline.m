function [Data,parameters] = fitBaranyiSaturationDrugDecline(Data,mu0,x0,q0,k0,g,d0,tin,tdrug,tfin,parameters)
...
%define the model
blackboxmodel = @ModelBaranyiSaturationDrugDecline;
...
%initial guess
P0 = [parameters.mu0,parameters.x0,parameters.q0,parameters.k0,parameters.g,parameters.d0,parameters.tdeath];
%eventual boundaries
xBoundaries = X(1:10);
Xmin = min(xBoundaries);
Xmax = max(xBoundaries);
lb = [-inf,Xmin,0,0,-inf,-inf,time(tdrug)];
ub = [inf,Xmax,inf,inf,inf,inf,time(tfin)];

%minimize chi square
[fittedParameters,fval] = fminsearchbnd(@(P) minimizeChiSquareSaturationDDecline(P,blackboxmodel,time,parameters,Data),P0,lb,ub,options);
...
parameters.d0 = fittedParameters(6);
parameters.tdeath = fittedParameters(7);
