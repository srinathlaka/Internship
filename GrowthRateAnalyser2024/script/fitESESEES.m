function [Data,parameters] = fitESESEES(Data,tin,tfin,parameters0,i,parameters)
...
%[1 klag,2 tlag,3 q0,4 mu0,5 tswitch,6 kswitch,7 kswitch2,8 -,9 -,10 tdeath,11 kdeath,12 m, 13 x0,14 k] 15 k2
param.mu = parameters0(4);
tswitch = parameters0(5);
param.kswitch = parameters0(6);
param.kswitch2 = parameters0(7);
tswitch2 = parameters0(8);
...
if isnan(param.kswitch2)
    param.kswitch2 = param.kswitch;
end

if isnan(parameters0(15))
    param.k2 = param.k;
else
...
%tswitch
if isnan(tswitch)
    tswitch = round(tin+(tswitch2-tin)/2);
    if tswitch>tswitch2
        tswitch=tswitch2;
    end
    param.tswitch = time(round(tin+(tswitch2-tin)/2));
else
...
%initial guess
P0 = [param.mu,param.x0,param.k,param.kdeath,param.tdeath,param.tswitch,param.kswitch,param.k2,param.kswitch2,param.tswitch2];
%eventual boundaries
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,Kmin,Kmin,time(tswitch2),time(tin),-inf,Kmin,-inf,time(tswitch)];
ub = [inf,Xmax,Kmax,Kmax,time(tfin),time(tswitch2),inf,Kmax,inf,time(tdeath)];

%model chi square
modelChi = @(P) minimizeChiSquareSD5(P,blackboxmodel,time,param,Data);

%minimize chi square
%[fittedparameters,fval] = fminsearchbnd(@(P) minimizeChiSquareSD5(P,blackboxmodel,time,param,Data),P0,lb,ub,options);
...
param.kswitch2 = fittedparameters(9);
param.tswitch2 = fittedparameters(10);
...
fprintf('mu = %f, x0 = %f, k = %f, k3 = %f, Tdeath = %f, Tswitch = %f, kswitch = %f, k2 = %f, kswitch2 = %f, tswitch2 = %f residual chi2/DOF = %f, p = %f \n',fittedparameters(1),fittedparameters(2),fittedparameters(3),fittedparameters(4),fittedparameters(5),fittedparameters(6),fittedparameters(7),fittedparameters(8),fittedparameters(9),fittedparameters(10),sigmaSq,p)
