function [Data,parameters] = fitLESED(Data,tin,tfin,parameters0,i,parameters)
...
%initial guess
P0 = [param.mu,param.x0,param.k,param.kdeath,param.tdeath,param.tswitch,param.kswitch,param.q0];
%eventual boundaries
...
Kmin = min(X);
Kmax = max(X);
lb = [0,Xmin,Kmin,-inf,time(tdeath),time(tin),-inf,0];
ub = [inf,Xmax,Kmax,inf,time(tfin),time(tswitch),inf,inf];
