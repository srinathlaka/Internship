function [Data,experiment] = ExperimentSubtractBkg4(experiment,bkg,averagecurves,bkg_subract,time_unit,file_excel)
...
for i=1:size(expRT,2)
    scatter(convert.*(experiment.time{1}),expRT(:,i),10,vectorcolor(i,:),'filled');
    %plot((experiment.time{1})./60,expRT(:,i));
    shadedErrorBar(convert.*(experiment.time{1}),expRT(:,i),Data.XRawNoBkgStd(:,i),'lineProps',{'color',vectorcolor(i,:)});
end
%set(gca,'YScale','log');
if time_unit==1
    xlabel('Time [min]')
...
ylabel('OD')
title('Single curve background subtracted')
grid on
box on
axis tight
