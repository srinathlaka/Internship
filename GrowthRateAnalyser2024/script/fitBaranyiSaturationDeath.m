function [Data,parameters] = fitBaranyiSaturationDeath(Data,mu0,x0,q0,k0,dm0,tin,tfin,parameters)
...
%define the model
blackboxmodel = @ModelBaranyiSaturationDeath;
...
%initial guess
P0 = [parameters.mu0,parameters.x0,parameters.q0,parameters.k0,parameters.dm0];
%eventual boundaries
lb = [];
ub = [];

%minimize chi square
[fittedParameters,fval] = fminsearchbnd(@(P) minimizeChiSquareSaturationDeath(P,blackboxmodel,time,parameters,Data),P0,lb,ub,options);

%compute p value
p = 1 - chi2cdf(fval,size(fittedParameters,2));
fprintf('mu = %f, x0 = %f, q0 = %f, k0 = %f, dm0 = %f, residual chi2 = %f, p = %f \n',fittedParameters(1),fittedParameters(2),fittedParameters(3),fittedParameters(4),fittedParameters(5),fval,p)
...
parameters.dm0 = fittedParameters(5);
