function [Data,parameters] = fitNoDrugLlinearSD3(Data,klag,mu,x0,k,kdeath,tin,tlag,tdeath,tswitch,tfin,kswitch,parameters)
...
%tswitch
if isempty(tswitch)
    parameters.tswitch = time(round(tlag+(tdeath-tlag)/2));
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,Kmin,0,time(tdeath),time(tlag),-inf,-inf,time(tin)];
