function [Data,parameters] = fitESESES(Data,tin,tfin,parameters0,i,parameters)
...
%[1 klag,2 tlag,3 q0,4 mu0,5 tswitch,6 kswitch,7 tswitch2,8 q02,9 kswitch2,10 tdeath,11 kdeath,12 m, 13 x0,14 k] [ 15 k2, 16 k3]
...
if isnan(parameters0(16))
    param.k3 = param.k;
else
    param.k3 = parameters0(16);
end
...
%initial guess
P0 = [param.mu,param.x0,param.k,param.kdeath,param.tdeath,param.tswitch,param.kswitch,param.k2,param.k3];
%eventual boundaries
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,Kmin,-inf,time(tswitch),time(tin),-inf,Kmin,Kmin];
ub = [inf,Xmax,Kmax,inf,time(tfin),time(tdeath),inf,Kmax,Kmax];

%model chi square
modelChi = @(P) minimizeChiSquarek3(P,blackboxmodel,time,param,Data);

%minimize chi square
%[fittedparameters,fval] = fminsearchbnd(@(P) minimizeChiSquarek3(P,blackboxmodel,time,param,Data),P0,lb,ub,options);
...
fprintf('mu = %f, x0 = %f, k = %f, kdeath = %f, Tdeath = %f, Tswitch = %f, kswitch = %f, k2 = %f, k3 = %f, residual chi2/DOF = %f, p = %f \n',fittedparameters(1),fittedparameters(2),fittedparameters(3),fittedparameters(4),fittedparameters(5),fittedparameters(6),fittedparameters(7),fittedparameters(8),fittedparameters(9),sigmaSq,p)
