function [Data,parameters] = fitBaranyiSaturationDrugSequenceDecline2(Data,x0,d0,tin,tdrug,tfin,parameters)
...
%initial guess
P0 = [parameters.x0,parameters.d0];
%eventual boundaries
xBoundaries = X(1:10);
...
%minimize chi square
[fittedParameters,fval] = fminsearchbnd(@(P) minimizeChiSquareSaturationSDecline2(P,blackboxmodel,time,parameters,Data),P0,lb,ub,options);

%compute p value
p = 1 - chi2cdf(fval,size(fittedParameters,2));
fprintf('x0 = %f, d0 = %f, residual chi2 = %f, p = %f \n',fittedParameters(1),fittedParameters(2),fval,p)
