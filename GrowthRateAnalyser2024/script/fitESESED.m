function [Data,parameters] = fitESESED(Data,tin,tfin,parameters0,i,parameters)
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,Kmin,-inf,time(tswitch2),time(tin),-inf,Kmin,-inf,time(tswitch)];
ub = [inf,Xmax,Kmax,inf,time(tfin),time(tswitch2),inf,Kmax,inf,time(tdeath)];
