function [Data,parameters] = fitEED(Data,tin,tfin,parameters0,i,parameters)
...
%initial guess
P0 = [param.mu,param.x0,param.kdeath,param.tdeath,param.tswitch,param.kswitch];
%eventual boundaries
xBoundaries = (X(X>0));%X(1:10);
Xmin = min(xBoundaries);
Xmax = max(xBoundaries);
lb = [-inf,Xmin,-inf,time(tswitch),time(tin),-inf];
ub = [inf,Xmax,inf,time(tfin),time(tdeath),inf];

%model chi square
modelChi = @(P) minimizeChiSquareSD2(P,blackboxmodel,time,param,Data);

%minimize chi square
%[fittedparameters,fval] = fminsearchbnd(@(P) minimizeChiSquareSD2(P,blackboxmodel,time,param,Data),P0,lb,ub,options);
