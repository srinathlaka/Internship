function [xPositions, yPositions, Label, RangeCut, FigHandles] = UnivarScatter(data, vectorcolor,stringMarker,varargin)
%--------------------------------------------------------------------------
...
vectorcolorT = vectorcolor(lm:lm+size(yValues,1)-1,:);
stringMarkerT = stringMarker(lm:lm+size(yValues,1)-1);

[yValues,y_SortingIndex]=sort(yValues);
vectorcolorSorted = vectorcolorT(y_SortingIndex,:);
stringMarkerSorted = stringMarkerT(y_SortingIndex);
...
% FigHandles{i}=scatter(xValues,yValues,PointSize,stringMarker(i),'MarkerEdgeColor', MarkerEdgeColor(EdgeIndex,:),'MarkerFaceColor','flat','CData',vectorcolor,'LineWidth',LineWidth);
for ll=1:size(xValues,1)
    scatter(xValues(ll),yValues(ll),PointSize,stringMarkerSorted(lt),'MarkerEdgeColor', MarkerEdgeColor(EdgeIndex,:),'MarkerFaceColor','flat','CData',vectorcolorSorted(lt,:),'LineWidth',LineWidth);
    hold on
    lm=lm+1;
    lt=lt+1;
end
