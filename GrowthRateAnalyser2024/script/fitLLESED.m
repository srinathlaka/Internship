function [Data,parameters] = fitLLESED(Data,tin,tfin,parameters0,i,parameters)
...
%tswitch
if isnan(tswitch)
    param.tswitch = time(round(tlag+(tdeath-tlag)/2));
    tswitch = round(tlag+(tdeath-tlag)/2);
else
...
%initial guess
P0 = [param.mu,param.x0,param.k,param.kdeath,param.tdeath,param.tswitch,param.kswitch,param.klag,param.tlag];
%eventual boundaries
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,Kmin,0,time(tswitch),time(tlag),-inf,-inf,time(tin)];
...
%minimize chi square
%[fittedparameters,fval] = fminsearchbnd(@(P) minimizeChiSquareLexpSD3(P,blackboxmodel,time,param,Data),P0,lb,ub,options);
...
param.klag = fittedparameters(8);
param.tlag = fittedparameters(9);
