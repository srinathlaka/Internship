function [Data,parameters] = fitNoDrugSDexp(Data,mu,x0,q0,k,kdeath,tin,tswitch,tfin,parameters)
...
%define the model
blackboxmodel = @ModelSaturationExpDeath;
...
Kmin = min(X);
Kmax = max(X);
lb = [-inf,Xmin,-inf,Kmin,-inf,time(tswitch)];
